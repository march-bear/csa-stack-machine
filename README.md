...
pos_integer ::= [ <any of "1-9"> ] { <any of "0-9"> }

integer ::= [ "-" ] pos_integer | "0"
...
+ данные определяются с помощью ключевого слова `word`, буферы - с помощью `buf`
+ код выполняется последовательно, начиная с первой строки, если секции не указаны, иначе начиная с первой строки после `section .code`
    + порядок может быть изменен с помощью безусловного или условных переходов
...
+ метки в качестве аргумента во время трансляции заменяются на соответствующие адреса
+ корректное завершение программы возможно только исполнением инструкции (`halt`)

Подробное описание операций дано в разделе [Набор инструкций](#набор-инструкций)
...
+ Процессор оперирует знаковыми 32-битными числами
+ Доступ к памяти осуществляется с помощью специальных регистров: `IP` для Program Mem и `AR` для Data Mem
    + значение `AR` устанавливается во время выполнения операций с памятью (`pop_by` или `push_by`)
+ Вершина стека отделена от самого стека для возможности подачи в АЛУ сразу двух значений со стека. Прямого доступа к ней нет. Целостность поддерживается на уровне модели
...
+ `term` - информация о связанном месте в исходном коде (строка, столбец, код)


## Транслятор
Интерфейс командной строки: `translator.py <input_file> [<target_file>]`

Если `<target_file>` не указан, имя целевого файла берется за `<input_file>.out`
...
+ Трансляция программы в машинный код, проверка корректности синтаксиса
    + строки разделяются на инструкцию и аргументы
    + проверяется корректность инструкции и аргументов
    + если проверка прошла успешно, команда добавляется в список
...
Сигналы:
+ `latch_<register>` - защелкнуть значение в регистре
