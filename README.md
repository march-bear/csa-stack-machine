...
| `swap`                         | SWAP               |       3       | Меняет местами два значения на вершине стека                                            |
| `mod2`                         | MOD2               |       1       | Берет значение с вершины стека и кладет на стек остаток от деления этого значения на 2  |
| `print`                        | PRINT              |       1       | Берет значение с вершины стека и отправляет его на порт вывода                          |
| `input`                        | INPUT              |       1       | Читает значение из порта ввода и кладет его на стек                                     |
| `jmp <addr>`                   | JMP `<addr>`       |       1       | Совершает переход на инструкцию по адресу `<addr>`                                      |
| `jz <addr>`                    | JZ `<addr>`        |       2       | Совершает переход на инструкцию по адресу `<addr>`, если на вершине стека 0             |
| `jg <addr>`                    | JG `<addr>`        |       2       | Совершает переход на инструкцию по адресу `<addr>`, если на вершине отрицательное число |
...
| `pop <addr>`                   | POP `<addr>`       |       7       | Берет значение с вершины стека и записывает его в память по адресу `<addr>`             |
| `halt`                         | HALT               |       0       | Останов                                                                                 |
